...
void __declspec(naked) naked_0x465087()
{
    __asm
    {
        ...
        movzx eax,byte ptr[ebp+0x10] // isQuest
        mov ecx,dword ptr[ebp+0x0C] // exp

        push eax // isQuest
        push ecx // exp
        push esi // user (killer)
        call user_skill::multiply_exp
        add esp,0xC

        // Actualizamos la experiencia en el stack para que el servidor use la nueva
        mov dword ptr[ebp+0x0C],eax

        movzx edx,byte ptr[ebp+0x10] // isQuest
        mov ecx,dword ptr[ebp+0x0C] // exp (multiplicada)
        mov eax,dword ptr[ebp+0x08] // victimId
        
        push edx // isQuest
        push ecx // exp
        push eax // victimId
        push esi // killer
        call webhook::handle_kill
        add esp,0x10
